...
for (const item of items) {
    const gameItems = item.toString().split(":");

    const gameId = +gameItems[0].split(" ")[1];
    const gameBag = gameItems[1].split(";");

    gamesSum += gameId;

    const gameBagsArr = gameBag.map((bag) => {
        const colors = bag.trim().split(", ");
        const colorsArr = [];

        for (const item of colors) {
            const splittedColor = item.split(" ");
            const obj = {
                [splittedColor[1]]: splittedColor[0],
            };
...
const filteredGames = games.filter((game) => {
    for (const _game of game.games) {
        if (_game["green"] > 13 || _game["red"] > 12 || _game["blue"] > 14) {
            return game;
        }
    }
});
